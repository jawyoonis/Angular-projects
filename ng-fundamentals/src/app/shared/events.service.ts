...
import { IEvents, ISession } from './model';
...
export class EventsService {
  ...
  return this.http.get<IEvents[]>('/api/events')
  .pipe(catchError(this.handleError<IEvents[]>('getEvents', [])));
  }
  ...
  updateEvent(event: any){
    let indext = this.Events.findIndex((x: any) => x=event.id);
    this.Events[indext]=event
  ...
  searchSessions(searchTerm:string){
   ...
   this.Events.forEach(event => {
      var machingSessions: any = event.sessions?.filter(session => 
    ...
    setTimeout(() => {emitter.emit(results);
    }, 100);
    return emitter
  }
  Events : IEvents[]= [
